...
class FinancialDataIndicator(models.Model):
    ...
    def _select_contraction_indicators(self):
        select_str = """
            SELECT
                p.company_id,
                p.id AS project_id,
    ...
    def _select_cash_flow_indicators(self):
        select_str = """
            SELECT
                p.company_id,
                p.id AS project_id,   
                ...
                p.essence_project AS name,
                fc.date AS date,
                fc.amount_in_company_currency AS amount,
                p.currency_id AS currency_id,
                CASE
                    WHEN p.amount_untaxed_in_company_currency != 0 THEN (p.amount_untaxed_in_company_currency - p.cost_price_in_company_currency) / p.amount_untaxed_in_company_currency
                    ELSE 0
                END AS profitability,
                ROUND(COALESCE(dc.amount_in_company_currency, 0), c.decimal_places) AS distribution,        
                'cash_flow' AS type,
                CAST(NULL AS integer) AS planned_acceptance_flow_id,
                CAST(NULL AS integer) AS planned_cash_flow_id,
                ...
                LEFT JOIN
                (
                    SELECT fact_cash_flow_id, SUM(amount_in_company_currency) AS amount_in_company_currency
                ...
                LEFT JOIN
                (
                    SELECT fact_cash_flow_id, SUM(amount) AS amount, SUM(amount_in_company_currency) AS amount_in_company_currency
                    FROM project_budget_distribution_cash
                    GROUP BY fact_cash_flow_id
                ) AS dc ON dc.fact_cash_flow_id = fc.id    	    
                ...
                SELECT
                    ...
                    p.essence_project AS name,
                    pc.date AS date,
                    ROUND(pc.amount_in_company_currency, c.decimal_places) AS amount,
                    p.currency_id AS currency_id,
                    CASE
                        WHEN p.amount_untaxed_in_company_currency != 0 THEN (p.amount_untaxed_in_company_currency - p.cost_price_in_company_currency) / p.amount_untaxed_in_company_currency
                        ELSE 0
                    END AS profitability,
                    ROUND(dc.amount_in_company_currency, c.decimal_places) AS distribution,
                    'cash_flow' AS type,
                    CAST(NULL AS integer) AS planned_acceptance_flow_id,
                ...
                LEFT JOIN
                (
                    SELECT planned_cash_flow_id, SUM(amount) AS amount, SUM(amount_in_company_currency) AS amount_in_company_currency
                    FROM project_budget_distribution_cash
                ...
                AND psfp.sale_figure_id = sf.res_id
                WHERE p.step_status = 'project' AND p.project_have_steps = false AND p.active = true 
                AND ROUND(pc.amount - COALESCE(dc.amount, 0), c.decimal_places) != 0
                UNION
                SELECT
                    ps.company_id,
                    p.id AS project_id,        
    ...
    def _select_gross_revenue_indicators(self):
        select_str = """
            SELECT
                p.company_id,
                p.id AS project_id,        
                ...
                p.essence_project AS name,
                fa.date AS date,
                ROUND(fa.amount_in_company_currency, c.decimal_places) AS amount,
                p.currency_id AS currency_id,
                CASE
                    WHEN p.amount_untaxed_in_company_currency != 0 THEN (p.amount_untaxed_in_company_currency - p.cost_price_in_company_currency) / p.amount_untaxed_in_company_currency
                    ELSE 0
                END AS profitability,
                ROUND(COALESCE(da.amount_in_company_currency, 0), c.decimal_places) AS distribution,
            ...
            LEFT JOIN
            (
                SELECT fact_acceptance_flow_id, SUM(amount) AS amount, SUM(amount_in_company_currency) AS amount_in_company_currency
            ...
            SELECT
                ...
                p.essence_project AS name,
                pa.date AS date,        
                ROUND(pa.amount_in_company_currency, c.decimal_places) AS amount,        
                p.currency_id AS currency_id,
                CASE
                    WHEN p.amount_untaxed_in_company_currency != 0 THEN (p.amount_untaxed_in_company_currency - p.cost_price_in_company_currency) / p.amount_untaxed_in_company_currency
                    ELSE 0
            ...
            LEFT JOIN
            (
                SELECT planned_acceptance_flow_id, SUM(amount) AS amount, SUM(amount_in_company_currency) AS amount_in_company_currency
                FROM project_budget_distribution_acceptance
            ...
            AND psfp.sale_figure_id = sf.res_id
            WHERE p.step_status = 'project' AND p.project_have_steps = false AND p.active = true
            AND ROUND(pa.amount - COALESCE(da.amount, 0), c.decimal_places) != 0
            UNION
            SELECT
                ps.company_id,
                p.id AS project_id,        
    ...
    def _select_margin_indicators(self):
        select_str = """
            SELECT
                p.company_id,
                p.id AS project_id,        
                ...
                CASE
                    ...
                    WHEN p.amount_untaxed_in_company_currency != 0 THEN ROUND((fa.amount_in_company_currency * (p.amount_untaxed_in_company_currency - p.cost_price_in_company_currency) / p.amount_untaxed_in_company_currency), c.decimal_places)
                ELSE 0
                END AS amount,
                p.currency_id AS currency_id,
                CASE
            ...
            SELECT
                ...
                CASE
                    WHEN fa.margin_manual_input = true THEN fa.margin
                    WHEN ps.amount_untaxed_in_company_currency != 0 THEN ROUND((fa.amount_in_company_currency * (ps.amount_untaxed_in_company_currency - ps.cost_price_in_company_currency) / ps.amount_untaxed_in_company_currency), c.decimal_places) 
                ELSE 0
                END AS amount,
                ps.currency_id AS currency_id,
                CASE
...
""" % (self._select_contraction_indicators(), self._select_cash_flow_indicators(),
            self._select_gross_revenue_indicators(), self._select_margin_indicators())
