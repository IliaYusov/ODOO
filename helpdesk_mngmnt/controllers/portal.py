...
class CustomerPortal(portal.CustomerPortal):
    ...
    def portal_my_helpdesk_tickets(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all',
                                   search=None, groupby='none', search_in='name', **kw):
        values = self._prepare_my_tickets_values(page, date_begin, date_end, sortby, filterby, search, groupby,
                                                 search_in)
        return request.render('helpdesk_mngmnt.portal_helpdesk_ticket', values)
    ...
    def tickets_followup(self, ticket_id=None, access_token=None, **kw):
        ...
        return request.render("helpdesk_mngmnt.portal_helpdesk_ticket_form", values)
    ...
    def _ticket_get_searchbar_groupby(self):
        return {
            ...
            'partner_id': {'label': _('Customer'), 'sequence': 50}
        }

    def _ticket_get_search_domain(self, search_in, search):
        if search_in == 'name':
            return ['|', ('name', 'ilike', search), ('code', 'ilike', search)]
