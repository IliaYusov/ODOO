...
class AuthOauthProvider(models.Model):
    ...
    ], string='Auth Flow', default='access_token', required=True)
    ...
    client_secret = fields.Char(help='Used in OpenID Connect authorization code flow for confidential clients.')
    code_verifier = fields.Char(default=lambda self: secrets.token_urlsafe(32), help='Used for PKCE.')
    validation_endpoint = fields.Char(required=False)
    token_endpoint = fields.Char(string='Token URL', help='Required for OpenID Connect authorization code flow.')
