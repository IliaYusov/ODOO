...
class Project(models.Model):
    ...
    amount_untaxed = fields.Monetary(string='Amount Untaxed', compute='_compute_spec_totals', readonly=False, store=True, tracking=True)
    cost_price = fields.Monetary(string='Cost Price', compute='_compute_spec_totals', readonly=False, store=True, tracking=True)
    ...
    def _calculate_all_sums(self, project):
        if not project.project_have_steps:
            ...
            project.with_context(form_fix_budget=True).taxes_fot_premiums = taxes_fot_premiums
