...
class License(models.Model):
    ...
    def unlink(self):
        undeletable = self.filtered(lambda rec: rec.workflow_process_id)
        if undeletable:
            raise UserError(
                _("The licenses '%s' cannot be deleted because workflow's were started. However, you can archive it.",
                  ','.join(undeletable.mapped('code'))))
