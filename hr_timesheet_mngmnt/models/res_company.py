...
class ResCompany(models.Model):
    ...
    timesheet_account_id = fields.Many2one('account.account', string='Account', check_company=True,
                                           domain=lambda self: [('company_id', '=', self.env.company.id)],
                                           help='Default account for timesheets')
    ...
    def write(self, values):
        for company in self:
            if 'analytic_plan_id' in values:
                self.env['ir.config_parameter'].sudo().set_param("default_analytic_plan_id_%d" % company.id, values['analytic_plan_id'])
