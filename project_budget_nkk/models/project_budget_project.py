...
class Project(models.Model):
    ...
    def _get_default_end_sale_project_month(self):
        return False if self.env.company.id == 3 else fields.Date.context_today(self)

    industry_id = fields.Many2one(readonly=False, related=False, required=True, store=True)
    ...
    technological_direction_id = fields.Many2one('project_budget.technological_direction',
                                                 string='Technological Direction', copy=True, tracking=True)
    ...
    margin = fields.Monetary(string='Margin', compute='_compute_totals', store=True)
    ...
    planned_cost_flow_ids = fields.One2many('project_budget.planned_cost_flow', 'projects_id',
                                            string='Planned Cost Flow', copy=True)
            ...
            if project.project_have_steps:
                for step in project.step_project_child_ids:
                    if (step.stage_id.code in ('50', '75', '100')
                        ...
                        raise ValidationError(raisetext)
                    # СА, ЛИТР
                    if step.company_id.id in (7, 10) and step.stage_id.code in ('50', '75') \
                            and not step.planned_step_cost_flow_ids \
                            and not (step.is_child_project and not step.margin_from_children_to_parent) \
                            and not step.is_correction_project:
                        raisetext = _('Please enter forecast for cost to project {0} step {1}').format(
                            step.step_project_parent_id.project_id, step.project_id)
                        raise ValidationError(raisetext)
            else:
                if (project.stage_id.code in ('50', '75', '100')
                    ...
                    raise ValidationError(raisetext)
                # СА, ЛИТР
                if project.step_status == 'project' and project.company_id.id in (7, 10) \
                        and project.stage_id.code in ('50', '75') \
                        and not (project.is_parent_project and project.margin_from_children_to_parent) \
                        and not (project.is_child_project and not project.margin_from_children_to_parent) \
                        and not project.is_correction_project and not project.planned_cost_flow_ids:
                    raisetext = _('Please enter forecast for cost to project {0}').format(project.project_id)
                    raise ValidationError(raisetext)

    @api.constrains('end_presale_project_month', 'end_sale_project_month')
    def _check_opportunity_date_end_greater(self):
        for rec in self.filtered(lambda pr: pr.budget_state == 'work' and pr.company_id.id != 3):
            if rec.end_sale_project_month < rec.end_presale_project_month:
                raise ValidationError(_("The opportunity's start date must be before its end date."))
    ...
    def check_project_overdue(self, project, vals_dict):
        if project.company_id.id != 5:  # не ТОПС
    ...
    def check_project_overdue_dates(self, project, vals_dict, stage_code):
        ...
        if project.company_id.id != 3:  # не Ландата
    ...
    def _calculate_amounts_in_company_currency(self, project):
        ...
        if not project.project_have_steps:
            if project.currency_id == project.company_currency_id:
                project.with_context(form_fix_budget=True).cost_price_in_company_currency = project.cost_price
                project.with_context(form_fix_budget=True).margin_in_company_currency = project.margin
            else:
                project.with_context(form_fix_budget=True).cost_price_in_company_currency = project.cost_price * project.currency_rate
                project.with_context(form_fix_budget=True).margin_in_company_currency = project.margin * project.currency_rate
            ...
            project.with_context(form_fix_budget=True).cost_price_in_company_currency = cost_price_in_company_currency
            project.with_context(form_fix_budget=True).margin_in_company_currency = margin_in_company_currency
    ...
    def _calculate_totals(self, project):
        ...
        if not project.project_have_steps:
            project.with_context(form_fix_budget=True).margin = project.amount_untaxed - project.cost_price
            project.with_context(form_fix_budget=True).profitability = (project.margin / project.amount_untaxed * 100) if project.amount_untaxed else 0
        elif project.project_have_steps and project.step_status == 'project':
            cost_price = 0
            margin = 0
            for step in project.step_project_child_ids:
            ...
            project.with_context(form_fix_budget=True).cost_price = cost_price
            project.with_context(form_fix_budget=True).margin = margin
            project.with_context(form_fix_budget=True).profitability = profitability
    ...
    @api.depends('step_project_parent_id.technological_direction_id', 'step_project_parent_id.project_supervisor_id',
                 'step_project_parent_id.industry_id')
    def _compute_related_fields(self):
        super(Project, self)._compute_related_fields()
        for rec in self.filtered(lambda pr: pr.step_status == 'step'):
            ...
            rec.industry_id = rec.step_project_parent_id.industry_id
    ...
    def _copy_flows_and_distributions(self, copied_project, mapping_old_to_new_steps, flow_type):
        ...
        for flow in self['fact_' + flow_type + '_flow_ids']:
            ...
            if flow['planned_' + flow_type + '_flow_id']:
                copied_flow['planned_' + flow_type + '_flow_id'] = mapping_old_to_new_flows.get(
                    flow['planned_' + flow_type + '_flow_id'].id, False)
