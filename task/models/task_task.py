...
class Task(models.Model):
    ...
    user_ids_domain = fields.Binary(compute='_compute_user_ids_domain')
    ...
    date_deadline = fields.Datetime(string='Deadline', required=True, copy=True, index=True, tracking=True)
    ...
    root_task_id = fields.Many2one('task.task', string='Root Task', compute='_compute_root_task_id', store=True)
    subtask_count = fields.Integer(string='Sub-task Count', compute='_compute_subtask_count')
    closed_subtask_count = fields.Integer(string='Closed Sub-tasks Count', compute='_compute_subtask_count')
    ...
    def _compute_subtask_planned_hours(self):
        for rec in self:
            rec.subtask_planned_hours = sum(
                child_task.planned_hours + child_task.subtask_planned_hours for child_task in rec.child_ids)
    ...
    def _compute_root_task_id(self):
        for rec in self:
            root_parent_id = int(rec.parent_path.split('/')[0])
            rec.root_task_id = False if root_parent_id == rec.id else root_parent_id

    @api.depends('company_id')
    def _compute_user_ids_domain(self):
        for rec in self:
            domain = [
                ('company_ids', 'in', rec.company_ids.ids)
            ]
            rec.user_ids_domain = domain
