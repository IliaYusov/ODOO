...
class budget_plan_supervisor(models.Model):
    ...
    responsibility_center_id = fields.Many2one('account.analytic.account', string='Project Office', copy=True,
    ...
    def name_get(self):
        result = []
        for rec in self:
            name = str(rec.year) + ' ' + (rec.responsibility_center_id.name or rec.company_id.name)
