...
class budget_plan_kam(models.Model):
    ...
    responsibility_center_id = fields.Many2one(related='plan_supervisor_id.responsibility_center_id')
    ...
    @api.depends('key_account_manager_id', 'year')
    def _get_name_to_show(self):
        for plan_kam in self:
            plan_kam.name_to_show = str(
                plan_kam.year) + ' ' + plan_kam.responsibility_center_id.name + ' ' + plan_kam.key_account_manager_id.name
