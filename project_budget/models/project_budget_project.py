...
class Project(models.Model):
    ...
    currency_rate = fields.Float(string='Currency Rate', compute='_compute_currency_rate', copy=True, precompute=True,
                                 store=True)
    ...
    isRukovoditel_required_in_project = fields.Boolean(related='responsibility_center_id.isRukovoditel_required_in_project',
    ...
    key_account_manager_id = fields.Many2one('hr.employee', string='Key Account Manager',
                                             ...
                                             inverse='_inverse_key_account_manager_id', check_company=True, copy=True,
    ...
    responsibility_center_id = fields.Many2one('account.analytic.account', string='Responsibility Center',
                                               check_company=True, copy=True, depends=['key_account_manager_id'],
    ...
    partner_id = fields.Many2one('res.partner', string='Customer', copy=True, domain="[('is_company', '=', True)]",
                                 ondelete='restrict', required=True, tracking=True)
    industry_id = fields.Many2one('res.partner.industry', related='partner_id.industry_id', string='Industry',
                                  readonly=True, tracking=True)
    essence_project = fields.Text(string='essence_project', default = "",tracking=True)
    end_presale_project_month = fields.Date(string='Date of transition to the Production Budget(MONTH)',
    ...
    tax_id = fields.Many2one('account.tax', string='Tax', copy=True,
                             ...
                             domain="[('company_id', '=', company_id), ('type_tax_use', '=', 'sale')]", tracking=True)
    amount_total = fields.Monetary(string='Amount Total', compute='_compute_totals', store=True, tracking=True)
    amount_total_in_company_currency = fields.Monetary(string='Amount Total In Company Currency',
                                                       compute='_compute_amounts_in_company_currency',
                                                       currency_field='company_currency_id', store=True, tracking=True)
    amount_untaxed = fields.Monetary(string='Amount Untaxed', copy=True, store=True, tracking=True)
    amount_untaxed_in_company_currency = fields.Monetary(string='Amount Untaxed In Company Currency',
                                                         compute='_compute_amounts_in_company_currency',
    ...
    planned_acceptance_flow_ids = fields.One2many('project_budget.planned_acceptance_flow', 'projects_id',
                                                  string='Planned Acceptance Flows', copy=False)
    planned_amount_untaxed_acts = fields.Monetary(string='Planned Untaxed Amount Of Acts',
                                                  compute='_compute_planned_amount_acts', tracking=True)
    planned_amount_untaxed_in_company_currency_acts = fields.Monetary(string='Planned Untaxed Amount In Company Currency Of Acts',
                                                                      compute='_compute_planned_amount_acts',
                                                                      currency_field='company_currency_id',
                                                                      tracking=True)
    planned_amount_total_acts = fields.Monetary(string='Planned Amount With Tax Of Acts',
                                                compute='_compute_planned_amount_acts')
    planned_amount_total_in_company_currency_acts = fields.Monetary(string='Planned Amount With Tax In Company Currency Of Acts',
                                                                    compute='_compute_planned_amount_acts',
                                                                    currency_field='company_currency_id')

    fact_acceptance_flow_ids = fields.One2many('project_budget.fact_acceptance_flow', 'projects_id',
                                               string='Fact Acceptance Flows', copy=False)
    fact_amount_untaxed_acts = fields.Monetary(string='Fact Untaxed Amount Of Acts',
                                               compute='_compute_fact_amount_acts', tracking=True)
    fact_amount_untaxed_in_company_currency_acts = fields.Monetary(string='Fact Untaxed Amount In Company Currency Of Acts',
                                                                   compute='_compute_fact_amount_acts',
                                                                   currency_field='company_currency_id')
    fact_amount_total_acts = fields.Monetary(string='Fact Amount With Tax Of Acts',
                                             compute='_compute_fact_amount_acts', tracking=True)
    fact_amount_total_in_company_currency_acts = fields.Monetary(string='Fact Amount With Tax In Company Currency Of Acts',
                                                                 compute='_compute_fact_amount_acts',
                                                                 currency_field='company_currency_id')

    planned_cash_flow_ids = fields.One2many('project_budget.planned_cash_flow', 'projects_id',
                                            string='Planned Cash Flow', copy=False)
    planned_amount_total_cash_flow = fields.Monetary(string='Planned Amount With Tax Of Cash Flow',
                                                     compute='_compute_planned_amount_cash_flow', tracking=True)
    planned_amount_total_in_company_currency_cash_flow = fields.Monetary(string='Planned Amount With Tax In Company Currency Of Cash Flow',
                                                                         compute='_compute_planned_amount_cash_flow',
                                                                         currency_field='company_currency_id')

    fact_cash_flow_ids = fields.One2many('project_budget.fact_cash_flow', 'projects_id', string='Fact Cash Flow',
                                         copy=False)
    fact_amount_total_cash_flow = fields.Monetary(string='Fact Amount With Tax Of Cash Flow',
                                                  compute='_compute_fact_amount_cash_flow', tracking=True)
    fact_amount_total_in_company_currency_cash_flow = fields.Monetary(string='Fact Amount With Tax In Company Currency Of Cash Flow',
                                                                      compute='_compute_fact_amount_cash_flow',
                                                                      currency_field='company_currency_id')
    ...
    step_project_child_ids = fields.One2many('project_budget.projects', 'step_project_parent_id', string='Steps',
                                             compute='_compute_related_fields', copy=False, readonly=False, store=True)
    ...
    def _calculate_amounts_in_company_currency(self, project):
        if not project.project_have_steps:
            if project.currency_id == project.company_currency_id:
                project.with_context(form_fix_budget=True).amount_total_in_company_currency = project.amount_total
                project.with_context(form_fix_budget=True).amount_untaxed_in_company_currency = project.amount_untaxed
            else:
                project.with_context(form_fix_budget=True).amount_total_in_company_currency = project.amount_total * project.currency_rate
                project.with_context(form_fix_budget=True).amount_untaxed_in_company_currency = project.amount_untaxed * project.currency_rate
            ...
            project.with_context(form_fix_budget=True).amount_total_in_company_currency = amount_total_in_company_currency
            project.with_context(form_fix_budget=True).amount_untaxed_in_company_currency = amount_untaxed_in_company_currency
    ...
    def _calculate_totals(self, project):
        if not project.project_have_steps:
            project.with_context(form_fix_budget=True).amount_total = project.amount_untaxed * (1 + project.tax_id.amount / 100)
        elif project.project_have_steps and project.step_status == 'project':
            amount_untaxed = 0
            amount_total = 0
            for step in project.step_project_child_ids:
            ...
            project.with_context(form_fix_budget=True).amount_untaxed = amount_untaxed
            project.with_context(form_fix_budget=True).amount_total = amount_total
    ...
    def _compute_responsibility_center_id_domain(self):
        for rec in self:
            ...
            if responsibility_centers:
                domain = [
                    ('id', 'in',
                     responsibility_centers.child_ids.ids if responsibility_centers.child_ids else responsibility_centers.ids)
                ]
    ...
    @api.depends('planned_acceptance_flow_ids.amount', 'planned_acceptance_flow_ids.amount_in_company_currency')
    def _compute_planned_amount_acts(self):
        for rec in self:
            rec.planned_amount_untaxed_acts = 0
            rec.planned_amount_untaxed_in_company_currency_acts = 0
            rec.planned_amount_total_acts = 0
            rec.planned_amount_total_in_company_currency_acts = 0
            if rec.step_status == 'project':
                rec.planned_amount_untaxed_acts = sum(rec.planned_acceptance_flow_ids.mapped('amount'))
                rec.planned_amount_untaxed_in_company_currency_acts = sum(
                    rec.planned_acceptance_flow_ids.mapped('amount_in_company_currency'))
                rec.planned_amount_total_acts = sum(
                    rec.step_project_child_ids.filtered(lambda pr: pr.stage_id.project_state != 'cancel').mapped(
                        'planned_amount_total_acts')) if rec.project_have_steps else rec.planned_amount_untaxed_acts * (
                        1 + rec.tax_id.amount / 100)
                rec.planned_amount_total_in_company_currency_acts = sum(
                    rec.step_project_child_ids.filtered(lambda pr: pr.stage_id.project_state != 'cancel')
                        .mapped('planned_amount_total_in_company_currency_acts')) if rec.project_have_steps else \
                        rec.planned_amount_untaxed_in_company_currency_acts * (1 + rec.tax_id.amount / 100)
            elif rec.step_status == 'step':
                rec.planned_amount_untaxed_acts = sum(rec.planned_step_acceptance_flow_ids.mapped('amount'))
                rec.planned_amount_untaxed_in_company_currency_acts = sum(
                    rec.planned_step_acceptance_flow_ids.mapped('amount_in_company_currency'))
                rec.planned_amount_total_acts = rec.planned_amount_untaxed_acts * (
                        1 + rec.tax_id.amount / 100)
                rec.planned_amount_total_in_company_currency_acts = rec.planned_amount_untaxed_in_company_currency_acts * (
                            1 + rec.tax_id.amount / 100)

    @api.depends('fact_acceptance_flow_ids.amount', 'fact_acceptance_flow_ids.amount_in_company_currency')
    def _compute_fact_amount_acts(self):
        for rec in self:
            rec.fact_amount_untaxed_acts = 0
            rec.fact_amount_untaxed_in_company_currency_acts = 0
            rec.fact_amount_total_acts = 0
            rec.fact_amount_total_in_company_currency_acts = 0
            if rec.step_status == 'project':
                rec.fact_amount_untaxed_acts = sum(rec.fact_acceptance_flow_ids.mapped('amount'))
                rec.fact_amount_untaxed_in_company_currency_acts = sum(
                    rec.fact_acceptance_flow_ids.mapped('amount_in_company_currency'))
                rec.fact_amount_total_acts = sum(
                    rec.step_project_child_ids.filtered(lambda pr: pr.stage_id.project_state != 'cancel').mapped(
                        'fact_amount_total_acts')) if rec.project_have_steps else rec.fact_amount_untaxed_acts * (
                        1 + rec.tax_id.amount / 100)
                rec.fact_amount_total_in_company_currency_acts = sum(
                    rec.step_project_child_ids.filtered(lambda pr: pr.stage_id.project_state != 'cancel').mapped(
                        'fact_amount_total_in_company_currency_acts')) if rec.project_have_steps else rec.fact_amount_untaxed_in_company_currency_acts * (
                            1 + rec.tax_id.amount / 100)
            elif rec.step_status == 'step':
                rec.fact_amount_untaxed_acts = sum(rec.fact_step_acceptance_flow_ids.mapped('amount'))
                rec.fact_amount_untaxed_in_company_currency_acts = sum(
                    rec.fact_step_acceptance_flow_ids.mapped('amount_in_company_currency'))
                rec.fact_amount_total_acts = rec.fact_amount_untaxed_acts * (1 + rec.tax_id.amount / 100)
                rec.fact_amount_total_in_company_currency_acts = rec.fact_amount_untaxed_in_company_currency_acts * (
                        1 + rec.tax_id.amount / 100)

    @api.depends('planned_cash_flow_ids.amount', 'planned_cash_flow_ids.amount_in_company_currency')
    def _compute_planned_amount_cash_flow(self):
        for rec in self:
            rec.planned_amount_total_cash_flow = 0
            rec.planned_amount_total_in_company_currency_cash_flow = 0
            if rec.step_status == 'project':
                rec.planned_amount_total_cash_flow = sum(rec.planned_cash_flow_ids.mapped('amount'))
                rec.planned_amount_total_in_company_currency_cash_flow = sum(
                    rec.planned_cash_flow_ids.mapped('amount_in_company_currency'))
            elif rec.step_status == 'step':
                rec.planned_amount_total_cash_flow = sum(rec.planned_step_cash_flow_ids.mapped('amount'))
                rec.planned_amount_total_in_company_currency_cash_flow = sum(
                    rec.planned_step_cash_flow_ids.mapped('amount_in_company_currency'))

    @api.depends('fact_cash_flow_ids.amount', 'fact_cash_flow_ids.amount_in_company_currency')
    def _compute_fact_amount_cash_flow(self):
        for rec in self:
            rec.fact_amount_total_cash_flow = 0
            rec.fact_amount_total_in_company_currency_cash_flow = 0
            if rec.step_status == 'project':
                rec.fact_amount_total_cash_flow = sum(rec.fact_cash_flow_ids.mapped('amount'))
                rec.fact_amount_total_in_company_currency_cash_flow = sum(
                    rec.fact_cash_flow_ids.mapped('amount_in_company_currency'))
            elif rec.step_status == 'step':
                rec.fact_amount_total_cash_flow = sum(rec.fact_step_cash_flow_ids.mapped('amount'))
                rec.fact_amount_total_in_company_currency_cash_flow = sum(
                    rec.fact_step_cash_flow_ids.mapped('amount_in_company_currency'))

    @api.depends('step_project_parent_id.company_id', 'step_project_parent_id.currency_id',
                 'step_project_parent_id.commercial_budget_id', 'step_project_parent_id.key_account_manager_id',
                 'step_project_parent_id.project_manager_id', 'step_project_parent_id.signer_id',
                 'step_project_parent_id.partner_id', 'step_project_parent_id.responsibility_center_id',
                 'step_project_parent_id.is_correction_project', 'step_project_parent_id.is_not_for_mc_report',
                 'step_project_parent_id.approve_state', 'step_project_parent_id.company_partner_id')
    def _compute_related_fields(self):
        for rec in self.filtered(lambda pr: pr.step_status == 'step'):
            rec.company_id = rec.step_project_parent_id.company_id
            rec.currency_id = rec.step_project_parent_id.currency_id
            rec.commercial_budget_id = rec.step_project_parent_id.commercial_budget_id
            rec.key_account_manager_id = rec.step_project_parent_id.key_account_manager_id
            rec.project_manager_id = rec.step_project_parent_id.project_manager_id
            rec.signer_id = rec.step_project_parent_id.signer_id
            rec.partner_id = rec.step_project_parent_id.partner_id
            rec.approve_state = rec.step_project_parent_id.approve_state
            if rec.step_project_parent_id.different_project_offices_in_steps:  # проверям меняли ли ПО в проекте-этапе
                try:
                    cur_id = int(str(rec.step_project_parent_id.id).replace('NewId_', ''))
                    cur_office = self.env['project_budget.projects'].search([
                        ('id', '=', cur_id)
                    ], limit=1).responsibility_center_id
                    if cur_office == rec.responsibility_center_id:
                        rec.responsibility_center_id = rec.step_project_parent_id.responsibility_center_id
                except:
                    pass
            else:
                rec.responsibility_center_id = rec.step_project_parent_id.responsibility_center_id
            rec.is_correction_project = rec.step_project_parent_id.is_correction_project
            rec.is_not_for_mc_report = rec.step_project_parent_id.is_not_for_mc_report
            rec.company_partner_id = rec.step_project_parent_id.company_partner_id


    @api.depends('project_member_ids.role_id')
    def _compute_key_account_manager_id(self):
        for rec in self:
            rec.key_account_manager_id = rec.project_member_ids.filtered(lambda t: t.role_id == self.env.ref(
                'project_budget.project_role_key_account_manager'))[:1].employee_id or False
            # responsibility_center = self._get_responsibility_center_by_employee(rec.key_account_manager_id)
            # rec.responsibility_center_id = responsibility_center.id or False
    ...
    @api.constrains('stage_id', 'planned_acceptance_flow_ids', 'planned_cash_flow_ids', 'planned_step_cash_flow_ids',
    ...
    def _check_allowed_forecast_stages(self):  # проверяем чтобы вероятности прогнозов были связаны с вероятностью проектов
        for rec in self:
            if rec.budget_state == 'work' and not rec.is_correction_project:
                raisetext = ''
                if rec.step_status == 'step':
                    accs_ids = rec.planned_step_acceptance_flow_ids
                    cash_ids = rec.planned_step_cash_flow_ids
                elif rec.step_status == 'project':
                    accs_ids = rec.planned_acceptance_flow_ids
                    cash_ids = rec.planned_cash_flow_ids
                for acceptance in accs_ids:
                    project = acceptance.projects_id if not acceptance.projects_id.project_have_steps else \
                        acceptance.step_project_child_id
                    ...
                    if raisetext:
                        raisetext = raisetext.format(acceptance.flow_id)
                        raise ValidationError(raisetext)
                for cash in cash_ids:
                    project = cash.projects_id if not cash.projects_id.project_have_steps else \
                        cash.step_project_child_id
                    ...
                    if raisetext:
                        raisetext = raisetext.format(cash.flow_id)
                        raise ValidationError(raisetext)
    ...
    def _reset_forecasts_stages(self):  # сбрасываем вероятности прогнозов до вероятности проекта при изменении последнего
        for project in self:
            ...
            for acceptance in accs_ids:
                if acceptance.forecast != 'from_project':
                    acceptance.forecast_probability_id = self.env.ref(
                        'project_budget_forecast.project_budget_forecast_probability_from_project').id
                    acceptance.forecast = 'from_project'
                    has_changed = True
            for cash in cash_ids:
                if cash.forecast != 'from_project':
                    cash.forecast_probability_id = self.env.ref(
                        'project_budget_forecast.project_budget_forecast_probability_from_project').id
    ...
    def check_project_overdue_plans(self, project, plans, facts, plan_type, steps_ids_to_skip, vals_dict):
        ...
        distribution_ids = set(plans[dist_ids].filtered(lambda d: d.amount > 0).ids)
                            ...
                            for dist_id, dist_change in changed_dists.items():
                                if dist_change['action'] == 0:  # создание распределения
                                    ...
                                    if plan_id in dist_change['vals']:  # если поменялся Прогноз для распределения
                                        if ('amount' not in dist_change['vals'] or
                                                dist_change['vals']['amount'] != 0):
                                            plans_with_new_distribution_ids.add(dist_change['vals'][plan_id])
                                        distribution_ids.discard(dist_id)
                                    elif 'amount' in dist_change['vals']:  # если поменялась сумма распределения
                                        if dist_change['vals']['amount'] == 0:
                                            distribution_ids.discard(dist_id)
                                        else:
            ...
            if plan_changes:
                if plan_changes['action'] == 2:  # план удален
                    ...
                    if plan_changes['vals']:
                        new_date = plan_changes['vals'].get('date')
                        if new_date:
    ...
    def set_approve_supervisor(self):
        for rows in self:
            if rows.approve_state == "need_approve_supervisor" and rows.budget_state == 'work' and rows.project_status != 'cancel':
                ...
                user_id = False
                if rows.responsibility_center_id.receive_tasks_for_approve_project: # не только куратор может утвекрждать, но и руководитель проектного офиса надо
                    if rows.responsibility_center_id.head_id.user_id: # вдруг просто галочка стоит, а пользователь не выбран
                        user_id = rows.responsibility_center_id.head_id.user_id.id

                if self.user_is_supervisor(rows.project_supervisor_id.id) or self.user_has_groups(
    ...
    def cancel_approve(self):
        for rec in self:
            if (rec.approve_state == "approved" or rec.approve_state == "need_approve_supervisor") \
                    and rec.budget_state == 'work' and rec.project_status != 'cancel':
                user_id = False
                if rec.responsibility_center_id.receive_tasks_for_approve_project: # не только куратор может утвекрждать, но и руководитель проектного офиса надо
                    if rec.responsibility_center_id.head_id.user_id: # вдруг просто галочка стоит, а пользователь не выбран
                        user_id = rec.responsibility_center_id.head_id.user_id.id

                if self.user_is_supervisor(rec.project_supervisor_id.id) or self.user_has_groups(
                        'project_budget.project_budget_admin') or self.env.user.id == user_id:
                    rec.with_context(cancel_approve=True).write({
    ...
    def reopen(self):
        ...
        for record in self:
            user_id = False
            if record.responsibility_center_id.receive_tasks_for_approve_project:  # не только куратор может утвекрждать, но и руководитель проектного офиса надо
                if record.responsibility_center_id.head_id.user_id:  # вдруг просто галочка стоит, а пользователь не выбран
                    user_id = record.responsibility_center_id.head_id.user_id.id

            if not (self.user_is_supervisor(record.project_supervisor_id.id) or self.user_has_groups(
                'project_budget.project_budget_group_project_fixed_editor') or self.env.user.id == user_id):
        ...
        for row in self:
            if not (row.env.context.get('form_fix_budget') or row.env.context.get('cancel_approve')):
                if row.approve_state == 'need_approve_manager':
                    isok, raisetext = row.check_overdue_date(vals)
                    if not isok:
                        raise ValidationError(raisetext)
    ...
    def _get_responsibility_center_by_employee(self, employee):
        responsibility_center = self.env['account.analytic.account'].search([
            ('id', '=', employee.department_id.responsibility_center_id.id or 0)
        ], limit=1)
        return responsibility_center if not responsibility_center.child_ids else self.env['account.analytic.account']
    ...
    def _get_conversion_rate(self, from_currency):
        currency_rate = self.project_currency_rates_ids.filtered(lambda c: c.currency_id.id == from_currency.id)[:1]
        return currency_rate.rate if currency_rate else 1.0
    ...
    def _arch_preprocessing(self, arch):
        ...
        help_message = '\n'.join('%s - %s' % (stage.code, stage.requirements) for stage in stages)
        arch_node = etree.XML(arch)
        for node in arch_node.xpath("//form/sheet/group/group/field[@name='stage_id']"):
            node.set('help', help_message)
        return etree.tostring(arch_node, encoding='utf-8')
    ...
    def action_copy_step(self):
        ...
        if self.date_actual:  # сделка в зафиксированном бюджете
            ...
            raise (ValidationError(raisetext))
        self.sudo().copy({'step_project_parent_id': self.step_project_parent_id.id})

    @api.model
    def auto_update_for_amounts_in_company_currency(self):

        @lru_cache(maxsize=100)
        def _get_currency_rate(company_id, company_currency_id, currency_id, date):
        ...
        for project in projects:
            currency_rate = _get_currency_rate(project.company_id, project.company_currency_id, project.currency_id, datetime.date.today())
        ...
        for acceptance in acceptances:
            currency_rate = _get_currency_rate(acceptance.company_id, acceptance.company_currency_id, acceptance.currency_id, datetime.date.today())
        ...
        for cash in cashes:
            currency_rate = _get_currency_rate(cash.company_id, cash.company_currency_id, cash.currency_id, datetime.date.today())
