...
class PlannedAcceptanceFlow(models.Model):
    ...
    distribution_acceptance_ids = fields.One2many('project_budget.distribution_acceptance',
                                                  'planned_acceptance_flow_id', string='Distribution Fact By Plan',
                                                  copy=False)
    ...
    @api.depends('amount', 'currency_rate', 'distribution_acceptance_ids.amount',
                 'distribution_acceptance_ids.amount_in_company_currency')
    def _compute_amount(self):
        for rec in self:
            rec.amount_distribution = sum(rec.distribution_acceptance_ids.mapped('amount'))
            rec.amount_distribution_in_company_currency = sum(
                rec.distribution_acceptance_ids.mapped('amount_in_company_currency'))
            rec.amount_remaining = rec.amount - sum(rec.distribution_acceptance_ids.mapped('amount'))
            rec.amount_remaining_in_company_currency = 0 if rec.amount_remaining == 0 else \
                rec.amount_in_company_currency - sum(
                    rec.distribution_acceptance_ids.mapped('amount_in_company_currency'))
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('flow_id') or vals['flow_id'] == '-':
                vals['flow_id'] = self.env['ir.sequence'].sudo().next_by_code('project_budget.planned_acceptance_flow')
    ...
    def name_get(self):
        result = []
        for rec in self:
            name = rec.date.strftime('%d/%m/%Y') + _(' | acceptance ') + rec.flow_id + _(
                ' | sum cash without vat ') + f'{rec.amount:_.2f}'
    ...
    def action_copy_flow(self):
        super(PlannedAcceptanceFlow, self).action_copy_flow({'flow_id': '-'})
...
class FactAcceptanceFlow(models.Model):
    ...
    distribution_acceptance_ids = fields.One2many('project_budget.distribution_acceptance', 'fact_acceptance_flow_id',
    ...
    amount_distribution_in_company_currency = fields.Monetary(string='Distribution Amount In Company Currency',
                                                              ...
                                                              currency_field='company_currency_id', precompute=True,
                                                              store=True)
    amount_remaining = fields.Monetary(string='Remaining Amount', compute='_compute_amount', copy=True, precompute=True,
                                       store=True)
    amount_remaining_in_company_currency = fields.Monetary(string='Remaining Amount In Company Currency',
                                                           compute='_compute_amount', copy=True,
                                                           currency_field='company_currency_id', precompute=True,
    ...
    @api.depends('amount', 'distribution_acceptance_ids.amount',
                 'distribution_acceptance_ids.amount_in_company_currency')
    def _compute_amount(self):
        for rec in self:
            rec.amount_distribution = sum(rec.distribution_acceptance_ids.mapped('amount'))
    ...
    def action_copy_flow(self):
        super(FactAcceptanceFlow, self).action_copy_flow({'distribution_acceptance_ids': None})
