...
class ReportBDDSExcel(models.AbstractModel):
                ...
                if 'sum' not in period:
                    if budget_items.get(project.company_id.id):
                        for pds_plan in pds_plan_list:
                            if period[0] <= pds_plan.date <= period[1] and (not account_type or pds_plan.account_type_id == account_type):
                                pds_is_present = True
                                project_data['income'][pds_plan['budget_item_id'].id]['periods'][period] += pds_plan.amount_in_company_currency
                        ...
                        for pds_plan in pds_plan_list:
                            if period[0] <= pds_plan.date <= period[1] and (not account_type or pds_plan.account_type_id == account_type):
                                pds_is_present = True
                                project_data['income'][0]['periods'][period] += pds_plan.amount_in_company_currency
                        for cost_plan in cost_plan_list:
                            if period[0] <= cost_plan.date <= period[1] and (not account_type or cost_plan.account_type_id == account_type):
                                pds_is_present = True
    ...
    def print_worksheet(self, workbook, budget, sheet_name, responsibility_center_ids, max_level, dict_formula, report_period, account_type):
        ...
        projects = self.env['project_budget.projects'].search([
            ...
            '|', '|', '|',
            ('id', 'in', [plan.projects_id.id for plan in self.env['project_budget.planned_cash_flow'].search([]) if period_limits[0] <=  plan.date <= period_limits[1] and (not account_type or plan.account_type_id == account_type)]),
            ('id', 'in', [plan.step_project_child_id.id for plan in self.env['project_budget.planned_cash_flow'].search([]) if period_limits[0] <=  plan.date <= period_limits[1] and (not account_type or plan.account_type_id == account_type)]),
