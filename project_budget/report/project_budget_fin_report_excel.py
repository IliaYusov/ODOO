...
class report_budget_excel(models.AbstractModel):
    ...
    def printworksheet(self,workbook,budget,namesheet,stateproject):
        ...
        responsibility_centers = self.env['account.analytic.account'].search([('plan_id', '=', self.env.ref('analytic_responsibility_center.account_analytic_plan_responsibility_centers').id)], order='name')  # для сортировки так делаем
        ...
        for responsibility_center in responsibility_centers:
            ...
            for spec in cur_budget_projects:
                ...
                if (spec.stage_id.code in probabitily_list) and (
                    (spec.end_presale_project_month.year >= YEARint and spec.end_presale_project_month.year <= year_end)
                        or (spec.end_sale_project_month and spec.end_sale_project_month.year >= YEARint and spec.end_sale_project_month.year <= year_end)
                        or (spec.end_presale_project_month.year <= YEARint and spec.end_sale_project_month and spec.end_sale_project_month.year >= year_end)):
                    row += 1
                    ...
                    column += 1
                    if spec.responsibility_center_id.print_rukovoditel_in_kb == False:
            ...
            if isFoundProjectsByCenter:
                ...
                total_amount_of_revenue += f'{xl_col_to_name(12)}{start_row_of_responsibility_center + 1}:{xl_col_to_name(12)}{end_row_of_responsibility_center + 1},'
