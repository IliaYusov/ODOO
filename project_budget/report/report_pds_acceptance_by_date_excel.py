...
class report_pds_acceptance_by_date_excel(models.AbstractModel):
    ...
    def get_sum_plan_pds_project(self, project, date_start, date_end):
        ...
        if pds_list:
            for pds in pds_list:
                if date_start <= pds.date <= date_end and pds.forecast in ('commitment', 'reserve', 'from_project'):
                    if pds.amount > 0:
                        sum_cash += max(pds.amount_remaining_in_company_currency, 0)
                    elif pds.amount < 0:
                        sum_cash += max(pds.amount_remaining_in_company_currency, 0)
        return sum_cash
    ...
    def get_sum_plan_acceptance_project(self, project, date_start, date_end):
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if date_start <= acceptance.date <= date_end and acceptance.forecast in ('commitment', 'reserve', 'from_project'):
                    sum_cash += acceptance.amount_remaining_in_company_currency
    ...
    def print_worksheet(self, workbook, budget, sheet_name, date_start, date_end, pds_accept, legal_entity_shift):
        ...
        if pds_accept == 'pds':
            cur_budget_projects = self.env[
                ...
                '|', ('id', 'in', [pds.projects_id.id for pds in self.env['project_budget.planned_cash_flow'].search([]) if date_start <= pds.date <= date_end]),
                ('id', 'in', [pds.step_project_child_id.id for pds in self.env['project_budget.planned_cash_flow'].search([]) if date_start <= pds.date <= date_end]),
                '|', '&', ('step_status', '=', 'step'),
            ...
            cur_budget_projects = self.env[
                'project_budget.projects'].search([
                ('commercial_budget_id', '=', budget.id),
                '|', ('id', 'in', [acc.projects_id.id for acc in self.env['project_budget.planned_acceptance_flow'].search([]) if date_start <= acc.date <= date_end]),
                ('id', 'in', [acc.step_project_child_id.id for acc in self.env['project_budget.planned_acceptance_flow'].search([]) if date_start <= acc.date <= date_end]),
                '|', '&', ('step_status', '=', 'step'),
                ('step_project_parent_id.stage_id.code', 'not in', ('0', '10')),
                ('step_status', '=', 'project'),
                ('stage_id.code', 'not in', ('0', '10')),
