...
class ReportPdsWeeklyPlanFactExcel(models.AbstractModel):
        ...
        for month_delta in months_tuple:  # месяцы от начала текущего квартала
            ...
            if month_delta == months_tuple[0]:  # начало и конец всего периода
                ...
                period_limits.append(
                    month_end + timedelta(days=31))  # добавляем месяц если следующий месяц в новом квартале
    ...
    def get_data_from_projects(self, projects, periods_dict, budget, budget_ids):
        ...
        for project_project_id in sorted(project_project_ids):
                ...
                for period, options in periods_dict.items():
                    if 'sum' not in period:
                        ...
                        for pds_fact in pds_fact_list:
                            if period[0] <= pds_fact.date <= period[1] and project.commercial_budget_id.id == budget.id:
                                pds_is_present = True
                                project_data[period]['fact'] += pds_fact.amount_in_company_currency

                        for pds_plan in pds_plan_list:
                            if (period[0] <= pds_plan.date <= period[1]
                                ...
                                if (pds_plan.amount_remaining > 0 and pds_plan.amount > 0
                                        or pds_plan.amount_remaining < 0 and pds_plan.amount < 0):  # учитываем отрицательный ПДС для Энсиса
                                    pds_is_present = True
                                    project_data[period]['commitment'] += pds_plan.amount_remaining_in_company_currency
            ...
            if pds_is_present:
                ...
                if (
                        current_project.company_id.partner_id.id == current_project.signer_id.id
                        and current_project.company_partner_id
                        and current_project.company_id.partner_id.id != current_project.company_partner_id.partner_id.id
                        and not current_project.is_parent_project
                        and current_project.company_partner_id.partner_id.id in group_company_ids
                ):
                    opportunity_type = 'vgo'
                elif (
                        current_project.company_partner_id.partner_id.id == current_project.signer_id.id
                        and current_project.company_id.partner_id.id != current_project.signer_id.id
                        and not current_project.is_parent_project
                        and current_project.company_partner_id.partner_id.id in group_company_ids
                ):
                    opportunity_type = 'other_legal_entity'
                else:
    ...
    def print_row(self, sheet, workbook, companies, responsibility_centers, actual_center_ids, row, data, periods_dict, level, max_level, dict_formula):
        ...
        company_type_format = workbook.add_format({
            'top': 2,
            'bottom': 2,
            'left': 1,
            'right': 1,
        ...
        for company in companies:
            if company.id not in dict_formula['company_ids']:
                ...
                dict_formula['commercial_lines'][company.id] = list()
                dict_formula['vgo_lines'][company.id] = list()
                dict_formula['other_legal_entity_lines'][company.id] = list()
                ...
                if center.id in actual_center_ids:
                    ...
                    if center.name in data[company.name]:
                        for project, content in data[company.name][center.name].items():
                            ...
                            column += 1
                            sheet.write_number(row, column, content['info']['amount_total'], cur_row_format_number)
                            column += 1
                            ...
                            if content['info']['opportunity_type'] == 'commercial':
                                dict_formula['commercial_lines'][company.id].append(row)
                            if content['info']['opportunity_type'] == 'vgo':
                                dict_formula['vgo_lines'][company.id].append(row)
                            if content['info']['opportunity_type'] == 'other_legal_entity':
                                dict_formula['other_legal_entity_lines'][company.id].append(row)
            ...
            if level == 1:
                commercial_lines = dict_formula['commercial_lines'].get(company.id, [])
                vgo_lines = dict_formula['vgo_lines'].get(company.id, [])
                other_legal_entity_lines = dict_formula['other_legal_entity_lines'].get(company.id, [])
                if any(vgo_lines) or any(other_legal_entity_lines):
                    if any(commercial_lines):
                        row = row + 1
                        sheet.merge_range(row, 0, row, 11, company.name + ' (коммерческие)', company_type_format)
                        self.print_vertical_sum_formula(sheet, row, commercial_lines, periods_dict, 12,
                                                        'format_company')
                    if any(vgo_lines):
                        row = row + 1
                        sheet.merge_range(row, 0, row, 11, company.name + ' (ВГО)', company_type_format)
                        self.print_vertical_sum_formula(sheet, row, vgo_lines, periods_dict, 12, 'format_company')
                    if any(other_legal_entity_lines):
                        row = row + 1
                        sheet.merge_range(row, 0, row, 11, company.name + ' (проекты др ЮЛ Холдинга)',
                                          company_type_format)
                        self.print_vertical_sum_formula(sheet, row, other_legal_entity_lines, periods_dict, 12,
                                                        'format_company')
                    company_name = company.name
                    if any(vgo_lines) and not any(other_legal_entity_lines):
                        company_name += ' (с учетом ВГО)'
                    elif not any(vgo_lines) and any(other_legal_entity_lines):
                        company_name += ' (с учетом проектов др. ЮЛ Холдинга)'
                    elif any(vgo_lines) and any(other_legal_entity_lines):
                        company_name += ' (с учетом ВГО и проектов др. ЮЛ Холдинга)'
                    sheet.write_string(dict_formula['company_ids'][company.id], 0, company_name, company_format)
    ...
    def printworksheet(self, workbook, budget, namesheet, responsibility_center_ids, max_level, multipliers,
        ...
        projects = self.env['project_budget.projects'].search([
            ...
            '|','|','|',
            ('id', 'in', [fact.projects_id.id for fact in self.env['project_budget.fact_cash_flow'].search([]) if fact.date >= period_limits[0] and fact.date <= period_limits[1]]),
            ('id', 'in', [plan.projects_id.id for plan in self.env['project_budget.planned_cash_flow'].search([]) if plan.date >= period_limits[0] and plan.date <= period_limits[1]]),
            ('id', 'in', [fact.step_project_child_id.id for fact in self.env['project_budget.fact_cash_flow'].search([]) if fact.date >= period_limits[0] and fact.date <= period_limits[1]]),
            ('id', 'in', [plan.step_project_child_id.id for plan in self.env['project_budget.planned_cash_flow'].search([]) if plan.date >= period_limits[0] and plan.date <= period_limits[1]]),
        ], order='project_id')
    ...
    def generate_xlsx_report(self, workbook, data, budgets):
        dict_formula = {
            'company_ids': {},
            'center_ids': {},
            'center_ids_not_empty': {},
            'commercial_lines': {},
            'vgo_lines': {},
            'other_legal_entity_lines': {}
        }
