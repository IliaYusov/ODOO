...
class report_budget_forecast_excel(models.AbstractModel):
    ...
    def isProjectinYear(self, project):
        ...
        if project:
            ...
            if (project.end_presale_project_month.year >= YEARint and project.end_presale_project_month.year <= YEARint + 2)\
                    or (project.end_sale_project_month and project.end_sale_project_month.year >= YEARint and project.end_sale_project_month.year <= YEARint + 2)\
                    or (project.end_presale_project_month.year <= YEARint and project.end_sale_project_month and project.end_sale_project_month.year >= YEARint + 2):
                return True
            for pds in project.planned_cash_flow_ids:
                if pds.date.year >= YEARint and pds.date.year <= YEARint + 2:
                    return True
            for pds in project.fact_cash_flow_ids:
                if pds.date.year >= YEARint and pds.date.year <= YEARint + 2:
                    return True
            for act in project.planned_acceptance_flow_ids:
                if act.date.year >= YEARint and act.date.year <= YEARint + 2:
                    return True
    ...
    def get_sum_fact_pds_project_month(self, project, year, month, without_distributions):
        ...
        for pds in pds_list:
            if (pds.date.month == month or not month) and pds.date.year == year:
                if without_distributions and pds.distribution_cash_ids:
                    continue
                sum_cash += pds.amount_in_company_currency
        return sum_cash
            ...
            if (pds.date.month == month or not month) and pds.date.year == year:
                ...
                if pds.forecast == 'from_project':
                    if stage_id_code in ('75', '100', '100(done)'):
                        sum_cash['commitment'] = sum_cash.get('commitment', 0) + pds.amount_in_company_currency
                    elif stage_id_code == '50':
                        sum_cash['reserve'] = sum_cash.get('reserve', 0) + pds.amount_in_company_currency
                else:
                    if stage_id_code != '0':
                        sum_cash[pds.forecast] = sum_cash.get(pds.forecast, 0) + pds.amount_in_company_currency
    ...
    def print_year_pds_project(self, sheet, row, column, project, year, row_format_number, row_format_number_color_fact, next):
        ...
        if year:
            ...
            for planned_cash_flow in planned_cash_flows:
                if planned_cash_flow.date.year == year:
                    sum_distribution_pds += planned_cash_flow.amount_distribution_in_company_currency
                    stage_id_code = project.stage_id.code

                    if planned_cash_flow.amount_remaining_in_company_currency > 0:
                        if planned_cash_flow.forecast == 'from_project':
                            if stage_id_code in ('75', '100', '100(done)'):
                                sum_ostatok_pds['commitment'] = sum_ostatok_pds.get('commitment', 0) + planned_cash_flow.amount_remaining_in_company_currency
                            elif stage_id_code == '50':
                                sum_ostatok_pds['reserve'] = sum_ostatok_pds.get('reserve', 0) + planned_cash_flow.amount_remaining_in_company_currency
                        else:
                            if stage_id_code != '0':
                                sum_ostatok_pds[planned_cash_flow.forecast] = sum_ostatok_pds.get(planned_cash_flow.forecast, 0) + planned_cash_flow.amount_remaining_in_company_currency
    ...
    def print_month_pds_project(self, sheet, row, column, month, project, year, row_format_number, row_format_number_color_fact, next):
        ...
        if month:
            ...
            for planned_cash_flow in planned_cash_flows:
                if planned_cash_flow.date.month == month and planned_cash_flow.date.year == year:
                    sum_distribution_pds += planned_cash_flow.amount_distribution_in_company_currency
                    stage_id_code = project.stage_id.code

                    if (planned_cash_flow.amount_remaining_in_company_currency > 0 and planned_cash_flow.amount > 0
                            or planned_cash_flow.amount_remaining_in_company_currency < 0 and planned_cash_flow.amount < 0):  # учитываем отрицательный ПДС для Энсиса
                        if planned_cash_flow.forecast == 'from_project':
                            if stage_id_code in ('75', '100', '100(done)'):
                                sum_ostatok_pds['commitment'] = sum_ostatok_pds.get('commitment', 0) + planned_cash_flow.amount_remaining_in_company_currency
                            elif stage_id_code == '50':
    ...
    def get_sum_fact_acceptance_project_quarter(self, project, year, element_name, without_distributions):
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if (not months or acceptance.date.month in months) and acceptance.date.year == year:
                    if without_distributions and acceptance.distribution_acceptance_ids:
                        continue
                    sum_cash += acceptance.amount_in_company_currency
        return sum_cash
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if (not months or acceptance.date.month in months) and acceptance.date.year == year:
                    if without_distributions and acceptance.distribution_acceptance_ids:
                        continue
                    sum_cash += acceptance.margin_in_company_currency * multiplier
        return sum_cash
            ...
            for acceptance in acceptance_list:
                if (not months or acceptance.date.month in months) and acceptance.date.year == year:
                    if acceptance.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_acceptance['commitment'] = sum_acceptance.get('commitment', 0) + acceptance.amount_in_company_currency
                        elif stage_id_code == '50':
                            sum_acceptance['reserve'] = sum_acceptance.get('reserve', 0) + acceptance.amount_in_company_currency
                    else:
                        if stage_id_code != '0':
                            sum_acceptance[acceptance.forecast] = sum_acceptance.get(acceptance.forecast, 0) + acceptance.amount_in_company_currency
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if (not months or acceptance.date.month in months) and acceptance.date.year == year:
                    ...
                    if acceptance.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_margin['commitment'] += acceptance.amount_in_company_currency * profitability * multiplier / 100
                        elif stage_id_code == '50':
                            sum_margin['reserve'] += acceptance.amount_in_company_currency * profitability * multiplier / 100
                    else:
                        if stage_id_code != '0':
                            sum_margin[acceptance.forecast] += acceptance.amount_in_company_currency * multiplier * profitability / 100
        return sum_margin
    ...
    def get_margin_forecast_from_distributions(self, planned_acceptance, margin_plan, project, multiplier):
        ...
        for distribution in planned_acceptance.distribution_acceptance_ids:
            if distribution.fact_acceptance_flow_id.amount_in_company_currency != 0:
                margin_distribution += distribution.fact_acceptance_flow_id.margin_in_company_currency * distribution.amount_in_company_currency / distribution.fact_acceptance_flow_id.amount_in_company_currency
    ...
    def get_sum_planned_acceptance_project_from_distribution(self, project, year, element_name):
        ...
        for planned_acceptance_flow in acceptance_list:
            ...
            if (not months or planned_acceptance_flow.date.month in months) and planned_acceptance_flow.date.year == year:

                sum_distribution_acceptance += planned_acceptance_flow.amount_distribution_in_company_currency

                if planned_acceptance_flow.amount_remaining_in_company_currency > 0:
                    if planned_acceptance_flow.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_ostatok_acceptance['commitment'] = sum_ostatok_acceptance.get('commitment',
                                                                                              0) + planned_acceptance_flow.amount_remaining_in_company_currency
                        elif stage_id_code == '50':
    ...
    def print_acceptance_potential(self, sheet, row, column, project, year, format):
        ...
        if potential_acceptances:
            for acceptance in potential_acceptances:
                if acceptance.amount_remaining_in_company_currency > 0:
                    year_acceptance_30 += acceptance.amount_remaining_in_company_currency
        elif project.stage_id.code == '30' and project.end_sale_project_month and project.end_sale_project_month.year == year:
    ...
    def print_row(self, sheet, workbook, companies, responsibility_centers, key_account_managers, stages, budget, row, level, responsibility_center_ids, systematica_forecast):
        ...
        ], order='partner_id, project_id asc')
                        ...
                        for spec in cur_budget_projects:
                            if (spec.id not in dict_formula['printed_projects']
                                ...
                                if self.env.company.name == 'Ландата':
                                    column += 1
                                    sheet.write_string(row, column, ', '.join(spec.order_line_ids.product_id.mapped('name')) or '', cur_row_format)
                                    column += 1
                                    sheet.write_string(row, column, ', '.join(spec.order_line_ids.product_category_id.head_id.mapped('name')) or '', cur_row_format)
                                    # column += 1
                                    # sheet.write_string(row, column, spec.stage_id.code, cur_row_format)
                                column += 1
                                sheet.write_string(row, column, spec.partner_id.name, cur_row_format)
                                column += 1
                                sheet.write_string(row, column, spec.company_partner_id.partner_id.name or '',
                                                   cur_row_format)
                                column += 1
    ...
    def printworksheet(self,workbook,budget,namesheet, estimated_probabilities, responsibility_center_ids, systematica_forecast, oblako_row, diff_name):
        ...
        if self.env.company.name == 'Ландата':
            sheet.write_string(row + 1, column, "Продукт", head_format_1)
            sheet.write_string(row + 2, column, "", head_format_1)
            sheet.set_column(column, column, 21.5)
            column += 1
            sheet.write_string(row + 1, column, "Ответственный за продукт", head_format_1)
            sheet.write_string(row + 2, column, "", head_format_1)
            sheet.set_column(column, column, 21.5)
            column += 1
        ...
        column += 1
        sheet.write_string(row + 1, column, 'Внутренний Партнер Холдинга' if self.env.user.has_group(
            'project_budget_nkk.project_budget_group_print_report_sa') else 'Партнер', head_format_1)
        sheet.write_string(row + 2, column, "", head_format_1)
        sheet.set_column(column, column, 21.5)
        column += 1
        sheet.write_number(row - 2, column - 7 if self.env.company.name == 'Ландата' else column - 1, 1,
                           summary_format_percent)
        sheet.write_number(row - 1, column - 7 if self.env.company.name == 'Ландата' else column - 1, 0.6,
                           summary_format_percent)
        ...
        column += 1
        if self.env.company.name == 'Ландата':
            sheet.write_string(row + 1, column, "Вероятность CRM", head_format_1)
            sheet.write_string(row + 2, column, "", head_format_1)
            column += 1
            sheet.write_string(row + 1, column, "Юрлицо, подписывающее договор", head_format_1)
            sheet.write_string(row + 2, column, "", head_format_1)
        ...
        sheet.set_column(10 if self.env.company.name == 'Ландата' else 5,
                         15 if self.env.company.name == 'Ландата' else start_column, False, False, {'hidden': 1, 'level': 1})
        column += 1
        # sheet.write_string(row, column, "", head_format)
        ...
        if systematica_forecast: #  Суммируем Систематику и Облако
            ...
            if dict_formula['other_legal_entities_lines'].get(dict_formula['systematica_id']):
                start_row = row + 2
                for signer, lines in dict_formula['other_legal_entities_lines'][dict_formula['systematica_id']][
                    'company'].items():
                    row += 1
                    sheet.merge_range(row, column, row, column + 4, signer, row_format_number_vgo)
                    sheet.set_row(row, False, False, {'hidden': 1, 'level': 2})
            ...
            if (dict_formula['vgo_lines'].get(dict_formula['systematica_id'])
                ...
                formula_itogo = '=' + '-'.join('{0}' + str(r) for r in (
                    ...
                    dict_formula['other_legal_entities_lines'].get(dict_formula['systematica_id'], {}).get('company',
                                                                                                           {}).get(
                        'line')
                ) if r)
        ...
        if dict_formula['companies_lines']:
            ...
            formula_plan = '=sum(' + ','.join(
                ('{0}' + str(c + 1)) for c in dict_formula['centers_lines']) + ')'  # увеличиваем все номера строк на 1
            sheet.set_row(row, False, False, {'hidden': 1, 'level': 1})
        ...
        row += 1
        column = 0
        sheet.merge_range(row, column, row, column + 4, 'ИТОГО: Расчетный План по отчету',
                          row_format_itogo_estimated_plan_left)
        self.print_estimated_rows(sheet, row, row_format_itogo_estimated_plan,
                                  row_format_itogo_estimated_plan_cross)
        ...
        if systematica_forecast:
            ...
            if (dict_formula['vgo_lines'].get(dict_formula['systematica_id'])
                ...
                row += 2
                column = 0
                sheet.merge_range(row, column, row, column + 4, 'ИТОГО: СА+Облако.ру (коммерческие)',
            ...
            # ИТОГО
            column = 0
            sheet.merge_range(row + 1, column, row + 1, column + 4,
                              'ИТОГО: СА+Облако.ру по отчету (с учетом ВГО и проектов др. ЮЛ Холдинга)',
                              row_format_number_itogo)
            sheet.set_row(row + 1, False, False, {'hidden': 1, 'level': 1})
            for col in start_range:
                sheet.write_string(row + 1, col, '', row_format_number_itogo)
            for col in full_range:
                formula = '=sum({2}{0},{3}{2}{1})'.format(report_row, oblako_row, xl_col_to_name(col),
                                                          "'Прогноз (Облако.ру)'!")
                sheet.write_formula(row + 1, col, formula, row_format_number_itogo)
            # расчетный план по отчету
            sheet.merge_range(row + 2, column, row + 2, column + 4,
                              'ИТОГО: СА+Облако.ру Расчетный План по отчету (с учетом ВГО и проектов др. ЮЛ Холдинга)',
                              row_format_itogo_estimated_plan_left)
            ...
            for type in plan_shift:  # кресты в планах
                for period, shift in plan_shift[type].items():
                    formula = '=sum({2}{0},{3}{2}{1})'.format(report_row + 1, oblako_row + 1, xl_col_to_name(shift),
                                                              "'Прогноз (Облако.ру)'!")
        ...
        if diff_name:
            sheet.merge_range(
                ...
                f'Разница Итого; План ' + (
                    '6+6' if use_6_6 else f'{YEARint}') + f' ({diff_name})/ Расчетный план до конца периода (на дату отчета)',
                row_format_diff
            )
    ...
    def generate_xlsx_report(self, workbook, data, budgets):
        ...
        start_column = 15 if self.env.company.name == 'Ландата' else 12
