...
class report_management_committee_excel(models.AbstractModel):
    ...
    def isProjectinYear(self, project):
        ...
        if project:
            ...
            if project.end_presale_project_month.year in years or (
                    project.end_sale_project_month and project.end_sale_project_month.year in years):
                return True
            for pds in project.planned_cash_flow_ids:
                if pds.date.year in years:
                    return True
            for pds in project.fact_cash_flow_ids:
                if pds.date.year in years:
                    return True
            for act in project.planned_acceptance_flow_ids:
                if act.date.year in years:
                    return True
    ...
    def get_sum_fact_pds_project_year_quarter(self, project, year, quarter, without_distributions):
        ...
        for pds in pds_list:
            if (not quarter or pds.date.month in months) and pds.date.year == year:
            ...
            if (not quarter or pds.date.month in months) and pds.date.year == year:
                ...
                if pds.forecast == 'from_project':
                    if stage_id_code in ('75', '100', '100(done)'):
                        sum_cash['commitment'] += pds.amount_in_company_currency
                    elif stage_id_code == '50':
                        sum_cash['reserve'] += pds.amount_in_company_currency
                    elif stage_id_code == '30':
                        sum_cash['potential'] += pds.amount_in_company_currency
                else:
                    if stage_id_code != '0':
                        sum_cash[pds.forecast] += pds.amount_in_company_currency
        return sum_cash
    ...
    def get_pds_forecast_from_distributions(self, sum, project, year, months):
        ...
        for planned_cash in pds_list:
            if (not months or planned_cash.date.month in months) and planned_cash.date.year == year:

                sum_distribution_pds += planned_cash.amount_distribution_in_company_currency

                stage_id_code = project.stage_id.code

                if (planned_cash.amount_remaining > 0 and planned_cash.amount > 0
                        or planned_cash.amount_remaining < 0 and planned_cash.amount < 0):  # учитываем отрицательный ПДС для Энсиса
                    if planned_cash.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_ostatok_pds['commitment'] += planned_cash.amount_remaining_in_company_currency
                        elif stage_id_code == '50':
                            sum_ostatok_pds['reserve'] += planned_cash.amount_remaining_in_company_currency
                        elif stage_id_code == '30':
                            sum_ostatok_pds['potential'] += planned_cash.amount_remaining_in_company_currency
                    else:
                        if stage_id_code != '0':
                            sum_ostatok_pds[planned_cash.forecast] += planned_cash.amount_remaining_in_company_currency
    ...
    def get_sum_fact_acceptance_project_year_quarter(self, project, year, quarter, without_distributions):
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if (not quarter or acceptance.date.month in months) and acceptance.date.year == year:
            ...
            for acceptance in acceptance_list:
                if (not quarter or acceptance.date.month in months) and acceptance.date.year == year:
                    ...
                    if not any(distribution.fact_acceptance_flow_id.margin_manual_input for distribution in
                        ...
                        sum_cash_without_manual += acceptance.margin_in_company_currency * multiplier
            ...
            for acceptance in acceptance_list:
                if (not quarter or acceptance.date.month in months) and acceptance.date.year == year:
                    ...
                    if acceptance.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_acceptance['commitment'] += acceptance.amount_in_company_currency
                        elif stage_id_code == '50':
                            sum_acceptance['reserve'] += acceptance.amount_in_company_currency
                        elif stage_id_code == '30':
                            sum_acceptance['potential'] += acceptance.amount_in_company_currency
                    else:
                        if stage_id_code != '0':
                            sum_acceptance[acceptance.forecast] += acceptance.amount_in_company_currency
    ...
    def get_margin_plan(self, project, year, quarter, multiplier):
        ...
        if acceptance_list:
            for acceptance in acceptance_list:
                if ((not quarter or acceptance.date.month in months)
                        and acceptance.date.year == year
                    ...
                    if acceptance.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            ...
                            sum_margin['potential'] += acceptance.amount_in_company_currency * profitability * multiplier / 100
                    else:
                        if stage_id_code != '0':
                            sum_margin[acceptance.forecast] += acceptance.amount_in_company_currency * profitability * multiplier / 100
        return sum_margin
    ...
    def get_act_forecast_from_distributions(self, sum, project, year, months):
        ...
        for planned_acceptance in acceptance_list:
            if (not months or planned_acceptance.date.month in months) and planned_acceptance.date.year == year:
                sum_distribution_acceptance += planned_acceptance.amount_distribution_in_company_currency
                stage_id_code = project.stage_id.code

                if planned_acceptance.amount_remaining > 0:
                    if planned_acceptance.forecast == 'from_project':
                        if stage_id_code in ('75', '100', '100(done)'):
                            sum_ostatok_acceptance['commitment'] += planned_acceptance.amount_remaining_in_company_currency
                        elif stage_id_code == '50':
                            sum_ostatok_acceptance['reserve'] += planned_acceptance.amount_remaining_in_company_currency
                        elif stage_id_code == '30':
                            sum_ostatok_acceptance['potential'] += planned_acceptance.amount_remaining_in_company_currency
                    else:
                        if stage_id_code != '0':
                            sum_ostatok_acceptance[
                                planned_acceptance.forecast] += planned_acceptance.amount_remaining_in_company_currency
            ...
            if (not months or planned_acceptance.date.month in months) and planned_acceptance.date.year == year:
                ...
                for distribution in planned_acceptance.distribution_acceptance_ids:
                    ...
                    if distribution.fact_acceptance_flow_id.amount_in_company_currency != 0:
                        margin_distribution += (distribution.fact_acceptance_flow_id.margin_in_company_currency
                                                * distribution.amount_in_company_currency
                                                / distribution.fact_acceptance_flow_id.amount_in_company_currency)
                stage_id_code = project.stage_id.code
    ...
    def print_quarter_planned_acceptance_project(self, sheet, row, column, element, project, responsibility_center, params, row_format_number, row_format_number_color_fact):
        ...
        if element in ('Q3', 'Q4'):
            ...
            if all(value == 0 for value in sum_q1.values()) and project.end_sale_project_month and project.end_sale_project_month.year == YEARint + 1 and project.end_sale_project_month.month in self.get_months_from_quarter('Q1'):  # если актирование 0, а месяц в нужном году, берем выручку
                if project.stage_id.code == '30':
                    sum_q1['potential'] = project.amount_untaxed_in_company_currency
            #
            if all(value == 0 for value in sum.values()) and project.end_sale_project_month and project.end_sale_project_month.year == YEARint + 1:  # если актирование 0, а месяц в нужном году, берем выручку
                if project.stage_id.code == '30':
            ...
            if all(value == 0 for value in sum.values()) and project.end_sale_project_month and project.end_sale_project_month.year == YEARint + 2:  # если актирование 0, а месяц в нужном году, берем выручку
                if project.stage_id.code == '30':
                    sum['potential'] = project.amount_untaxed_in_company_currency
    ...
    def calculate_quarter_planned_acceptance(self, element, project, responsibility_center, params):
        ...
        if element in ('Q1', 'Q2', 'Q3', 'Q4'):
            ...
            if project.stage_id.code not in ('0', '10'):
                ...
                if (all(value == 0 for value in sum_q1.values()) and project.end_sale_project_month and project.end_sale_project_month.year == YEARint + 1
                    ...
                    if params['report_with_projects']:
                        for spec in cur_budget_projects:
                            if spec.responsibility_center_id == responsibility_center:
                                ...
                                column += 1
                                sheet.write_string(row, column, spec.essence_project if spec.step_status == 'project' else '%s: %s' % (spec.step_project_parent_id.essence_project, spec.essence_project), cur_row_format)
                                column += 1
                                ...
                                column += 1
                                sheet.write_number(row, column, spec.amount_total_in_company_currency,
