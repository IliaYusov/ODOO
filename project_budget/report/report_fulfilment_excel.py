...
class ReportFulfilmentExcel(models.AbstractModel):
    ...
    def get_data_from_projects(self, projects, year):
        ...
        for prj in projects:
            ...
            for cash_plan in cash_plan_ids:
                if cash_plan.date.year == year:
                    if cash_plan.date.month in (1, 2, 3):
                        cash_plan_y1q1 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (4, 5, 6):
                        cash_plan_y1q2 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (7, 8, 9):
                        cash_plan_y1q3 += cash_plan.amount_remaining_in_company_currency
                    else:
                        cash_plan_y1q4 += cash_plan.amount_remaining_in_company_currency
                elif cash_plan.date.year == year + 1:
                    if cash_plan.date.month in (1, 2, 3):
                        cash_plan_y2q1 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (4, 5, 6):
                        cash_plan_y2q2 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (7, 8, 9):
                        cash_plan_y2q3 += cash_plan.amount_remaining_in_company_currency
                    else:
                        cash_plan_y2q4 += cash_plan.amount_remaining_in_company_currency
                elif cash_plan.date.year == year + 2:
                    if cash_plan.date.month in (1, 2, 3):
                        cash_plan_y3q1 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (4, 5, 6):
                        cash_plan_y3q2 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (7, 8, 9):
                        cash_plan_y3q3 += cash_plan.amount_remaining_in_company_currency
                    else:
                        cash_plan_y3q4 += cash_plan.amount_remaining_in_company_currency
                elif cash_plan.date.year == year + 3:
                    if cash_plan.date.month in (1, 2, 3):
                        cash_plan_y4q1 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (4, 5, 6):
                        cash_plan_y4q2 += cash_plan.amount_remaining_in_company_currency
                    elif cash_plan.date.month in (7, 8, 9):
                        cash_plan_y4q3 += cash_plan.amount_remaining_in_company_currency
                    else:
                        cash_plan_y4q4 += cash_plan.amount_remaining_in_company_currency
                elif cash_plan.date.year > year + 3:  #
                    cash_plan_rest += cash_plan.amount_remaining_in_company_currency
            ...
            for acc_fact in acc_fact_ids:
                acceptance_fact_amount += acc_fact.amount * (1 + prj.tax_id.amount / 100)
    ...
    def print_worksheet(self, workbook, sheet, budget, year):
        ...
        for col in chain((8,),range(11, 30), (38, 39, 41)):  # суммирование ИТОГО
