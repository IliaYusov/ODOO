...
class WorkflowProcessActivity(models.Model):
    ...
    def process(self, signal=None):
        ...
        if self.state == 'completed' and (not signal.get('result', False) or signal[
            ...
            for tr in self.activity_id.out_transition_ids:
                check_condition = self._run_python_expression(tr.condition, signal)
                if check_condition:
    ...
    def process_task_result(self, signal=None):
        ...
        if signal.get('feedback', False):
            self.res_ref.with_context(without_document=True) \
                .message_post(body=signal.get('feedback'), message_type='comment', subtype_xmlid='mail.mt_note',
    ...
    def _run_python_code(self):
        return self._run_python_expression(self.activity_id.code)

    # TODO: как православно передать результат обработки задачи (согласовали/отклонили)? Нужен ли сигнал?
    def _run_python_expression(self, expression, signal=None):
    ...
    def _run_task(self):
        ...
        task_data = dict(
            ...
            description=self.env.context.get('default_description', False) or self.res_ref._fields.get('description',
                                                                                                       False),
    ...
    def _compute_task_assignees(self):
        ...
        if self.activity_id.type == 'task':
            if self.activity_id.user_ids:
                ...
                executor = self._run_python_expression(self.activity_id.auto_substitution_id.expression)
                if not executor:
    ...
    def _issue_rights(self, user_ids):
        result = False
        if self.res_ref._fields.get('access_ids', False):
            new_users = []
